...
void revertAnaglyph(char *file, char *imageType){
    ...
    cvZero(complement);
    int count = 0;
    int k = imageSize-1; //iterate through the CIT
    for(int row = 0; row < complement->height; row++){
        ...
        for(int col = 0; col < complement->width; col++){
            //Left Image
            ptrC[3*col] = cit[count]; //luminance
    ...
    //save reversed image 
    cvSaveImage("stereo-reversed.bmp", stereoPair);
    printf("Ok!\n");       
...
void createAnaglyph(char *file, IplImage *frameL, IplImage *frameR){
    ...
    //Copy luminance
    printf("Calculating luminance Y from complement... ");
    int count = 0;
    for(int row = 0; row < complement->height; row++){
        //set pointer to the correct position in each row
