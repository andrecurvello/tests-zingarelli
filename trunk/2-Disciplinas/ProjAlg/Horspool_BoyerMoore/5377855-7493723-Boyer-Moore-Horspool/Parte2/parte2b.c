...
void horspool(palavra *listaPalavras, int qtde, char *texto, int tamTexto){
    ...
    for(i=qtde-1; i>=0 && countPalavras<100; i--){//para cada palavra
...
void boyerMoore(palavra *listaPalavras, int qtde, char* texto, int tamTexto){    
   ...
   tamTexto-=count;
    ...
    for (i=qtde-1;i>=0 && countPalavras<100;i--){
...
int main(int argc, char*argv[]){
    ...
    strcpy(arquivo,"parte2b.txt");   

    ordenaFrequenciaPalavrasValidas(palavrasValidas,QTDE_PALAVRAS);
    imprime100MaisFrequentesPalavrasValidas(palavrasValidas,QTDE_PALAVRAS,arquivo);   
