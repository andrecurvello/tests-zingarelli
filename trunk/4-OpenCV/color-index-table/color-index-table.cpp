        ...
        for(int row = 0; row < anaglyph->height; row++){
            ...
            for(int col = 0; col < anaglyph->width; col++){
                ...
                if(col%2 == 0){//even columns receive current Cb and Cr from CIT
                    k++;
                    //Left Image
                    ptrStp[3*col+1] = cit[k]; //Cb
                    ptrStp[3*col+2] = cit[k+(imageSize/2)]; //Cr
                    //Right Image
                    ptrStp[3*(col+anaglyph->width)+1] = cit[k+imageSize]; //Cb
                    ptrStp[3*(col+anaglyph->width)+2] = cit[k+((3*imageSize)/2)]; //Cr                
                }
...
void createCIT(char *file, IplImage *frameL, IplImage *frameR){
    ...
    for(int row = 0; row < frameL->height; row++){
        ...
        for(int col = 0; col < (frameL->width)/2; col++){
            //TODO: subsampling
            cit[count]                   = (ptrL[3*(col+k)+1] + ptrL[3*(col+k+1)+1])/2; //Left Cb
            cit[count+(imageSize/2)]     = (ptrL[3*(col+k)+2] + ptrL[3*(col+k+1)+2])/2; //Left Cr
            cit[count+imageSize]         = (ptrR[3*(col+k)+1] + ptrR[3*(col+k+1)+1])/2; //Right Cb
            cit[count+((3*imageSize)/2)] = (ptrR[3*(col+k)+2] + ptrR[3*(col+k+1)+2])/2; //Right Cr
            count++;
            k++;
        }
    }    
