...
int main(int argc, char* argv[]){
    ...
    if(!strcmp(argv[2],"-e")){
        ...
        if (entrada == NULL) {
            ...
            int countCbCr = 0;
            float *subDataCompl = (float*) malloc(imageSize*sizeof(float));
            for (int i = 0; i < headerAnaglyph.biHeight; i++) {
                if(i % 2 == 0){ //copia valores Y Cb e Cr das linhas pares
                    for (int j = 0; j < headerAnaglyph.biWidth; j++) {
                        subDataCompl[countCbCr]              = Cb[i][j];
                        subDataCompl[countCbCr+imageSize/2]  = Cr[i][j];
                        countCbCr++;
                    }
                }
                else{
                    ...
                    for(int j = 0; j < headerAnaglyph.biWidth; j++){
                        subDataCompl[countCbCr]              = (subDataCompl[countCbCr]+Cb[i][j])/2;
                        subDataCompl[countCbCr+imageSize/2]  = (subDataCompl[countCbCr+imageSize/2] + Cr[i][j])/2;
                        countCbCr++;
                    }
                }
            }
            //escrita no arquivo de saída
            fwrite(subDataCompl, sizeof(float),imageSize, cit);
            fclose(cit);
            ...
            int countY = 0;
            countCbCr = 0;
            float *subDataVM = (float*) malloc(2*imageSize*sizeof(float));
            for (int i = 0; i < headerAnaglyph.biHeight; i++) {
                if(i % 2 == 0){ //copia valores Y Cb e Cr das linhas pares
                    for (int j = 0; j < headerAnaglyph.biWidth; j++) {
                        subDataVM[countCbCr]              = Cb[i][j];
                        subDataVM[countCbCr+imageSize/2]  = Cr[i][j];
                        subDataVM[countY+imageSize]       = Y[i][j];
                        countY++;
                        countCbCr++;
                    }
                }
                    ...
                    countCbCr -= headerAnaglyph.biWidth;
                    for(int j = 0; j < headerAnaglyph.biWidth; j++){
                        subDataVM[countCbCr]              = (subDataVM[countCbCr]+Cb[i][j])/2;
                        subDataVM[countCbCr+imageSize/2]  = (subDataVM[countCbCr+imageSize/2] + Cr[i][j])/2;
                        subDataVM[countY+imageSize]       = Y[i][j];
                        countY++;
                        countCbCr++;
                    }
            ...
            //escrita no arquivo de saída
            fwrite(subDataVM, sizeof(float),2*imageSize, vm);
            fclose(vm);
    ...
    if(!strcmp(argv[2],"-d")){
        ...
        if (entrada == NULL) {
            ...
            float *dataStreamCIT = (float*)malloc(imageSize*sizeof(float));
            fread(dataStreamCIT,sizeof(float),imageSize,cit);
            //leitura do anáglifo verde-magenta
            vm = fopen("verdemagenta.dat","rb");
            float *dataStreamVM = (float*)malloc(2*imageSize*sizeof(float));
            fread(dataStreamVM,sizeof(float),2*imageSize,vm);
