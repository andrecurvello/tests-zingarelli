...
int main(int argc, char* argv[]){
    ...
    saida = fopen("saida-arredondamento.bmp", "wb");
	...
	float a,b,c;
        ...
        int countY = 0;
        int countCbCr = 0;
        float *subData = (float*) malloc(3*imageSize*sizeof(float));
        for (int i = 0; i < header.biHeight; i++) {
        ...
        fwrite(subData, sizeof(float),3*imageSize, aux);
        fclose(aux);

        //YCbCr -> RGB
        aux = fopen("data.dat","rb");
        float *dataStreamOut = (float*)malloc(3*header.biHeight*header.biWidth*sizeof(float));
        fread(dataStreamOut,sizeof(float),3*imageSize,aux);
        //reversão para 4:4:4
        countY = 0;
        countCbCr = 0;
        for (int i = 0; i < header.biHeight; i++) {
	...
	float d, e, f;
        ...
        for (int i = 0; i < header.biHeight; i++) {
            for (int j = 0; j < header.biWidth; j++) {
                ...
                R[i][j] = arredondamento(d);
                G[i][j] = arredondamento(e);
                B[i][j] = arredondamento(f);
