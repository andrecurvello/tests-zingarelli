...
int main(int argc, char* argv[]){
    ...
    saida = fopen("saida-codigo6.bmp", "wb");
    ...
    if (entrada == NULL) {
        ...
        int countY = 0;
        int countCbCr = 0;
        char *subData = (char*) malloc(2*imageSize*sizeof(char));
        for (int i = 0; i < header.biHeight; i++) {
            if(i % 2 == 0){ //copia valores Y Cb e Cr das linhas pares
                for (int j = 0; j < header.biWidth; j++) {
                    ...
                    subData[countY+imageSize]       = (char)round(Y[i][j]);
                    countY++;
                    countCbCr++;
                }
            }
        ...
        fwrite(subData, sizeof(char),2*imageSize, aux);
        fclose(aux);

        //YCbCr -> RGB
        aux = fopen("data.dat","rb");
        char *dataStreamOut = (char*)malloc(2*header.biHeight*header.biWidth*sizeof(char));
        fread(dataStreamOut,sizeof(char),2*imageSize,aux);
        //reversão para 4:4:4
        countY = 0;
        countCbCr = 0;
        for (int i = 0; i < header.biHeight; i++) {
