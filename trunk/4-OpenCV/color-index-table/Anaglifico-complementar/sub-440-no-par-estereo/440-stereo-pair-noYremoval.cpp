/*
  ...
  Creation date: October 20th, 2011
  Last modification: October 20th, 2011

  Anaglyph conversion / reversion using YCbCr color space with 4:4:0 subsampling
  on both images of the stereo pair. On this implementatio, the Y component from
  the complementary anaglyph is not removed. This was a suggestion from one of
  the reviewers of an article we've submitted to ACM SAC 2011.
...
void reverseAnaglyph(char *file, char *imageType){
    ...
    char* newFile = (char*) malloc(sizeof(char)*(strlen(fileNoExtension)+20));
    ...
    uchar *cit = (uchar*) malloc(2*imageSize*sizeof(uchar));
    fread(cit, sizeof(uchar), 2*imageSize, fp);
    fclose(fp);
    free(newFile);
    printf("OK!\n");

    //Copy Y from main anaglyph -- we are not removing Y component on this implementation
    ...
    for(int row = 0; row < complement->height; row++){
        ...
        for(int col = 0; col < complement->width; col++){
            //complementary
            ptrC[3*col]   = cit[countY+imageSize]; //Y
...
void createAnaglyph(char *file, IplImage *frameL, IplImage *frameR){
    ...
    uchar *cit = (uchar*) malloc(2*imageSize*sizeof(uchar));
    cit = (uchar*)subsampling440(complement, WITH_Y);
    ...
    fwrite(cit, sizeof(uchar),2*imageSize, fp);
    fclose(fp);
    free(cit);
    free(newFile);
    cvReleaseImage(&complement);
